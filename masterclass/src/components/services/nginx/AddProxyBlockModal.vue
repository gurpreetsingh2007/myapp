<template>
  <div class="fixed inset-0 bg-white flex items-center justify-center z-50 p-8 transition-all duration-500 ease-out">
    <div
      ...
      <div v-if="showParamsModal"
        ...
        <div
          ...
          <div class="primary-gradient text-white p-6 flex justify-between items-center">
            ...
            <button @click="showParamsModal = false"
              class="w-10 h-10 rounded-xl hover:bg-white/20 transition-all duration-200 flex items-center justify-center group">
              <svg xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5 group-hover:rotate-90 transition-transform duration-200" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
          ...
          <div class="flex flex-col lg:flex-row flex-1 overflow-hidden">
            ...
            <div class="w-full lg:w-3/5 flex flex-col overflow-hidden" v-if="selectedLocationId !== null">
              ...
              <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
                <div class="space-y-4">
                  <div v-for="(directive, index) in locations[selectedLocationId].directives" :key="index"
                    ...
                    <div class="flex items-center gap-3">
                      <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div class="relative">
                          <input v-model="directive.param_name" type="text"
                        ...
                        <div class="relative">
                          <input v-model="directive.param_value" type="text"
                            class="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#005188] focus:border-[#005188] transition-all duration-200 bg-slate-50 focus:bg-white"
                            placeholder="Directive value" />
                        </div>
                      </div>
                      <button @click="removeDirective(index)"
                ...
                <div v-if="locations[selectedLocationId].directives.length === 0" class="text-center py-16">
              ...
              <div class="p-6 border-t border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                <div class="flex gap-3">
                  <div class="flex-1 relative">
                    <input v-model="newDirective.param_name" type="text" placeholder="Directive name"
                  ...
                  <div class="flex-1 relative">
                    <input v-model="newDirective.param_value" type="text" placeholder="Directive value"
                      class="w-full px-4 py-3 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#005188] focus:border-[#005188] transition-all duration-200 bg-white"
                      @keyup.enter="addCustomDirective" />
                  </div>
                  <button @click="addCustomDirective"
      ...
      <div
        ...
        <div class="relative flex top-3 mt-auto  items-center justify-around ">
          ...
          <button @click="$emit('close')" class="p-3  rounded-lg hover:bg-white/20 transition-all duration-300 group">
            <svg xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6 group-hover:rotate-90 transition-transform duration-300" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        <div
          ...
          <!-- Server Title -->
          <div class="flex flex-col">
            <label class="flex items-center space-x-3 text-sm font-semibold uppercase tracking-wider text-white/80">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="relative">
              <input v-model="serverTitle" type="text" placeholder="Enter server title (e.g., Coopolis)"
          ...
          <div class="flex flex-col">
            ...
            <div class="relative">
              <input v-model="serverPort" type="number" placeholder="443"
          ...
          <div class="flex flex-col">
            ...
            <div class="space-y-3">
              <label v-for="option in protocolOptions" :key="option"
                ...
                <div class="relative">
                  <input type="radio" name="protocol" :value="option" v-model="selectedProtocol"
                  ...
                  <div
                    ...
                    <div v-if="selectedProtocol === option"
                      class="w-4 h-4 rounded-full bg-white shadow-lg animate-pulse"></div>
                  </div>
                </div>
              </label>
            </div>
          ...
          <div class="flex flex-col" v-if="selectedProtocol !== 'HTTP'">
            ...
            <div class="relative">
              <select v-model="selectedCertificate"
                ...
                <option v-for="cert in certificates" :key="cert" :value="cert" class="bg-gray-800 text-white indent-2">
                  {{ cert?.split('/').pop() }}
                </option>
              </select>
              <svg class="absolute right-4 top-1/2 transform -translate-y-1/2 h-6 w-6 text-white/60 pointer-events-none"
        ...
        <div class="pt-4 p-2 border-t  border-white/20">
          <button @click="saveConfiguration"
            ...
            <svg xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6 group-hover:rotate-12 transition-transform duration-300" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
      ...
      <div class="w-[65%] bg-gradient-to-br    flex flex-col  from-gray-50 to-white relative">
        ...
        <div class="relative flex   items-center justify-around ">
          <div class="flex items-center space-x-4 mb-3">
            <div>
              <h2 class="text-4xl font-bold text-gray-800 tracking-tight">{{ serverTitle || 'New Server Configuration'
              }}</h2>
              <p class="text-lg text-gray-600 mt-2">Configure server names, locations, and parameters</p>
            </div>
          </div>
        </div>
          ...
          <div
            ...
            <div class="flex items-center justify-between  relative p-2">
              <h3 class="text-2xl gap-2 font-bold text-gray-800 flex items-center ">
                <div class="w-10 h-10 bg-[#005188] rounded-xl flex items-center justify-center shadow-md ">
            ...
            <div class="p-2 ">
              <div class="relative ">
                ...
                <svg class="absolute right-5 top-1/2 transform -translate-y-1/2 h-6 w-6 text-gray-400" fill="none"
                  viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="flex p-2 pt-3">
                <input v-model="newServerName" @keyup.enter="addServerName" type="text"
              ...
              <div v-if="filteredServerNames.length > 0"
                ...
                <div v-for="(name, index) in filteredServerNames" :key="index"
                  ...
                  <button @click="removeServerName(index)"
          ...
          <div
            ...
            <div class="flex items-center justify-between p-2 relative">
              <h3 class="text-2xl font-bold gap-2 text-gray-800 flex items-center space-x-4">
                <div class="w-10 h-10 bg-[#007C52] rounded-xl flex items-center justify-center shadow-md">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                ...
                <div class="flex items-start space-x-6">
                  <div class="flex-grow space-y-6">
                    <div class="flex items-center space-x-4">
                      ...
                      <input v-model="location.path" type="text" placeholder="/api/v1 or /example"
                    ...
                    <div class="flex items-center space-x-4">
                      ...
                      <input v-model="location.proxy_pass" type="text"
          ...
          <div
            ...
            <div class="space-y-6">
              <div v-if="parameters.length === 0" class="text-center">
                ...
                <p class="text-sm text-gray-400 mt-2">Click "Add Parameter" to configure server settings</p>
              </div>
              ...
              <div v-for="(param, index) in parameters" :key="index"
                ...
                <div class="grid grid-cols-12 gap-4 items-center">
                  <div class="col-span-5">
                    <label class="block text-sm font-medium text-gray-600 mb-2">Parameter Name</label>
                    <input v-model="param.param_name" type="text" placeholder="e.g., max_connections, timeout"
                  ...
                  <div class="col-span-5">
                    <label class="block text-sm font-medium text-gray-600 mb-2">Parameter Value</label>
                    <input v-model="param.param_value" type="text" placeholder="redirect"
...
import type { Server, Parameter as StoreParameter } from '@/stores/services/nginx/nginx'
...
// Computed property to get unique parameters - fixed logic
const uniqueLocationParameters = computed(() => {
  const seen = new Set()
  return nginxStore.locationParameters.filter((param) => {
    const key = `${param.param_name}:${param.param_value}`
...
const isParameterActive = (param: { param_name: string; param_value: string }) => {
  ...
  return location.directives.some(d =>
    d.param_name === param.param_name &&
    d.param_value === param.param_value
  )
}
...
const toggleParameter = (param: { param_name: string; param_value: string; is_common?: boolean }) => {
  ...
  if (existingIndex >= 0) {
    ...
    location.directives.push({
      ...
      is_common: param.is_common // Preserve the is_common flag
    })
  }
}
...
const addCustomDirective = () => {
  ...
  newDirective.value = { param_name: '', param_value: '', is_common: false }
}
...
const resetToDefaults = () => {
  ...
  // Reset to only common parameters
  const commonDefaults = uniqueLocationParameters.value
    .filter(p => p.is_common)
    .map((p) => ({ ...p }))

  locations.value[selectedLocationId.value].directives = commonDefaults
}
...
// Add newDirective ref that was missing
const newDirective = ref<Directive>({ param_name: '', param_value: '', is_common: false })
...
const certificates = computed(() =>
  nginxStore.certificates.map(cert => cert.cert_name)
)
...
// Helper to create a new location object - fixed to only include common parameters
const createNewLocation = (): Location => ({
  path: '',
  proxy_pass: '',
  directives: nginxStore.locationParameters
    .filter(p => p.is_common === true)
    .map(p => ({
  ...
  try {
    ...
    const config: Omit<Server, 'server_id'> = {
      ...
      ssl_certificate: selectedProtocol.value !== 'HTTP'
        ? matchedCert.value?.cert_path
        : '',
      ssl_certificate_key: selectedProtocol.value !== 'HTTP'
        ? matchedCert.value?.key_path
        : '',
...
onMounted(async () => {
  try {
    ...
    // Reinitialize locations after parameters are loaded to ensure proper default selection
    locations.value = [createNewLocation()]
  } catch (error) {
